...
function    [HSI_res, PSNR,RMSE, ergas, sam, ssim, sz]    =    NSSR_HSI_SR( Dir, image, sf, kernel_type,blur_delta )
rand('seed',0);
time0           =    clock;
[Z_ori,sz]      =    load_HSI( Dir, image, sf );
par             =    Parameters_setting( sf, kernel_type, sz,blur_delta );
X               =    par.H(Z_ori);
par.P           =    create_P();
Y               =    par.P*Z_ori;
...
HSI_res = min(HSI_res,1);
HSI_res = max(HSI_res,0);
[PSNR,RMSE, ergas, sam, uiqi,ssim,DD,CCS] = quality_assessment(reshape(Z_ori,sz(1),sz(2),31), reshape(HSI_res,sz(1),sz(2),31), 0, 1/sf);
% disp(psnr,rmse, ergas, sam,ssim);
